...
// MEDIC DM NOISE
#define RECHARGE_NOISE 	    "PDA.SniperBeep" // This noise plays when a clients jump recharges.
// ====[ VARIABLES ]===================================================
...
public void OnPluginStart()
{
    ...
    // ADDED FOR MEDIC
    PrecacheScriptSound(RECHARGE_NOISE);
    HookEvent("player_healed", Event_player_heal);
    HookEvent("player_changeclass", Event_player_changeclass);
    CreateTimer(5.0, ResetHP, _, TIMER_REPEAT);
    // ====]
...
public void OnClientDisconnect(int client) {
    ...
    // MEDIC DM
    SDKUnhook(client, SDKHook_OnTakeDamage, SDKEvent_TakeDamage);
}
...
public Action Event_player_spawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	// MEDIC DM STUFF ===]
		...
		// Don't let them switch off of Medic.
    if (GetClientTeam(client) != 7)
	{
		TF2_SetPlayerClass(client, TFClass_Medic);
	}
		// Set their health to 10 so the 'player_healed' event can fire when teammates hit each other.
    SetEntityHealth(client, 10);
		
		// Make it so they can only use crossbow.
...
===================================]
===================================]
MEDIC ONLY FUNCTIONS. ADDED 7-9-22.
===================================]
===================================]
**/

// Timer function to constantly lower HP.
/*

The way I handle people on the same team killing each other is by killing them when the "player_healed" event fires.
This event doesn't fire if the player is full health though. To circumvent this we constantly set each players HP to 10.

*/
Action ResetHP(Handle timer)
{
...
Action Timer_ResetJump(Handle timer, int client)
{
	g_djUsed[client] = false;
	EmitGameSoundToClient(client, RECHARGE_NOISE, client);
}

// Event that fires when a player presses anything.
// Add fun jump mechanic. :)
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if ((buttons & IN_RELOAD) && !g_djUsed[client]) // If the reload button is pressed and the player has a double jump available then...
	{
		g_djUsed[client] = true;
		float cVel[3];
		GetEntPropVector(client, Prop_Data, "m_vecVelocity", cVel); // Get the players current velocity and store it in "cVel[3]".
		
		ScaleVector(cVel, 2.00); // Scale the velocity by 2.
		
		cVel[2] += 280.00; // Y-velocity is very tiny so lets give it a boost to get some height.
		
		TeleportEntity(client, NULL_VECTOR, NULL_VECTOR, cVel); // Implement the jump.
		CreateTimer(3.0, Timer_ResetJump, client); // Wait 3 seconds and allow the client to jump again.
	}
	
	return Plugin_Continue;
}

 /* Event_player_changeclass()
 *
 * Called when a attemps to change class.
 * -------------------------------------------------------------------------- */
 public Action Event_player_changeclass(Handle event, const char[] name, bool dontBroadcast)
 {
 	...
 	// No switching off of Medic.
 	if (class != 7)
 	{
 		TF2_SetPlayerClass(client, TFClass_Medic);
 		PrintToChat(client, "[SM] Medic is the only class allowed.");
 	}
 	return Plugin_Continue;
 }
 ...
 /* SDKEvent_TakeDamage()
 *
 * SDKHook when a client takes damage. This exists to prevent fall damage.
 * -------------------------------------------------------------------------- */
 public Action SDKEvent_TakeDamage(int client, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if (!IsValidClient(client)) { return Plugin_Handled; }
	
	// If fall damage. Block the event.
	if (damagetype & DMG_FALL)
	{
		return Plugin_Handled;
	}
	
	// Otherwise kill them. All players are Medic so don't worry about checking the damage source.
	SDKHooks_TakeDamage(client, inflictor, attacker, 450.00);
...
/* OnClientPutInServer()
 *
 * SDKHook each client when they're put in the server.
 *-------------------------------------------------------------------------- */
public void OnClientPutInServer(int client)
{
    ...
    if (IsValidClient(client))
    {
    	SDKHook(client, SDKHook_OnTakeDamage, SDKEvent_TakeDamage);
    }
}
